import { itemApi, groupBuyApi, postApi } from '@/api'
import type { ItemInfo, GroupBuyInfo, PostInfo } from '@/api/types'
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Link to="/secondhand"
              ...
              <p className="text-sm text-foreground-500 mb-3">校园闲置物品流转，让闲置变现金，省钱又环保</p>
              <span className="inline-flex items-center gap-1 text-sm text-primary-500 font-medium px-3 py-1.5 bg-primary-100 rounded-lg group-hover:bg-primary-200 transition-colors">
                浏览商品 <i className="ri-arrow-right-s-line transition-transform group-hover:translate-x-0.5"></i>
              </span>
            </Link>
            <Link to="/groupbuy"
              ...
              <p className="text-sm text-foreground-500 mb-3">组织拼团更优惠，一起买更便宜，省钱购物新方式</p>
              <span className="inline-flex items-center gap-1 text-sm text-accent-500 font-medium px-3 py-1.5 bg-accent-100 rounded-lg group-hover:bg-accent-200 transition-colors">
                查看拼团 <i className="ri-arrow-right-s-line transition-transform group-hover:translate-x-0.5"></i>
              </span>
            </Link>
            <Link to="/forum"
              ...
              <p className="text-sm text-foreground-500 mb-3">选课攻略、活动通知、学习交流，校园信息一站式</p>
              <span className="inline-flex items-center gap-1 text-sm text-secondary-500 font-medium px-3 py-1.5 bg-secondary-100 rounded-lg group-hover:bg-secondary-200 transition-colors">
                进入论坛 <i className="ri-arrow-right-s-line transition-transform group-hover:translate-x-0.5"></i>
              </span>
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/secondhand"
              ...
              查看全部 <i className="ri-arrow-right-s-line"></i>
            </Link>
          </div>
          {hotItems.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {hotItems.map((item) => (
                <ProductCard key={item.id} item={item} liked={likedIds.has(item.id)} onLikeToggle={handleLikeToggle} />
              ))}
            </div>
          ) : (
            <div className="text-center py-10">
              <p className="text-foreground-400 text-sm">暂无热门商品</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-12 px-4 bg-background-100">
        <div className="max-w-7xl mx-auto">
          ...
          {hotGroupBuys.length > 0 ? (
            ...
            <div className="text-center py-10">
              <p className="text-foreground-400 text-sm">暂无进行中的拼团</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          {hotPosts.length > 0 ? (
            ...
            <div className="text-center py-10">
              <p className="text-foreground-400 text-sm">暂无热门讨论</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-16 px-4 bg-background-100">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h2 className="text-xl font-semibold text-foreground-800 mb-3">让闲置流动起来</h2>
          ...
          <div className="flex justify-center gap-4">
            <Link to="/secondhand"
              ...
              <i className="ri-add-line"></i>发布闲置
            </Link>
            <Link to="/groupbuy"
              ...
              <i className="ri-group-line"></i>发起拼团
            </Link>
          </div>
        </div>
      </section>
